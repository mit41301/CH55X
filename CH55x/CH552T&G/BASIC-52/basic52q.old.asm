$mod51
;                                                                2023.05.04
; WCH CH552T/G 対応バージョン
;   Internal Clock 24MHz, CPU Clock(Fsys) 6MHz(Defalt)
;   Timer2 を UART0 のシリアルポート用クロック生成に使用。
;   シリアルポートの自動速度認識機能が正常に動かないので、起動時「Speed OK ?」の
;   メッセージを表示し正常に表示されたら「y」と答えることで対応。
;
;*****************************************************************************
;*                                                                           *
...
;
;=== CH552 Added =====
	orl	t2mod,#0C0H		; Set bTMR_CLK, bT2_CLK ( Set Timer2 Clock, MAX Speed = Fsys = 6MHz )
;=====================
...
;******* Wulf 4 **************************************************************
;
;-- comment out for AT98LP52/CH552 ---------------------------------------------
;
;BG1:	clr	a
;	mov	t2con,a
;	mov	TH2,#0FFh
...
;	-- CH552 --
;   RCAP2H,L = 65536 - (Fsys/16/Baud Rate) : When Fsys = 6MHz, bTMR_CLK=1,bT2_CLK=1
;
SPEED_TBL:	db	0ECH	; RCAP2H,L = FFECH (65516 : 18.75Kbps)
		db	0EDH	; RCAP2H,L = FFEDH (65517 : 19.737Kbps)
		db	0D9H	; RCAP2H,L = FFD9H (65497 :  9.615Kbps)
		db	00h	; Table End
;
